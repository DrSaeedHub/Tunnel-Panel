...
var routeDescriptions = map[string]routeSummary{
	...
	"POST /api/v1/tunnels/{id}/diagnostics/tcp":     {"Open a connection across the tunnel", "Knocks on the peer's address over the tunnel and reports what its IP stack said. A refused connection answers the question as well as an accepted one; only a reply that never came means the tunnel is not carrying traffic. Use it where ICMP is filtered and the ping measures the filter rather than the tunnel.", "diagnostics"},
	"GET /api/v1/system/tuning":                     {"Kernel parameters and what they should be", "Every parameter the panel understands, what this kernel holds now, and what this host should be setting it to \u2014 computed from its memory, its cores and the connections its rules are actually carrying.", "system"},
	"POST /api/v1/system/tuning/apply":              {"Apply the recommended values", "Sets them and records what each was first, in the panel's own sysctl file, so the whole set survives a reboot and can be put back.", "system"},
	"GET /api/v1/quota":                             {"Every traffic limit's standing", "What each limited tunnel, rule and destination is allowed, what it has used in the current window, and whether the panel has stopped it.", "system"},
	"PUT /api/v1/quota":                             {"Set or remove a traffic limit", "A limit of zero removes it; removing a limit that had stopped something starts it again.", "system"},
	"POST /api/v1/quota/reset":                      {"Start a limit's count over", "Rebases the usage to zero and starts the subject again if the panel had stopped it — the button for having bought more traffic.", "system"},
	"POST /api/v1/system/tuning/set":                {"Set the parameters by hand", "Keeps the values an operator chose rather than the recommended ones. Sending a parameter with an empty value asks the panel to stop keeping it, leaving the kernel with whatever it holds.", "system"},
	"POST /api/v1/system/tuning/revert":             {"Put the parameters back", "Restores what each was before the panel first changed it and removes the panel's tuning file.", "system"},
	"GET /api/v1/source-lists":                      {"The named address lists", "Each list with its ranges and how many forwarding rules allow it.", "routes"},
	"POST /api/v1/source-lists":                     {"Create an address list", "Ranges are parsed out of one block of text, however it is separated, so a paste and an uploaded file need no different handling.", "routes"},
	"GET /api/v1/source-lists/{id}":                 {"One address list", "The list with every range it holds.", "routes"},
	"PUT /api/v1/source-lists/{id}":                 {"Replace an address list", "The ranges are replaced, not added to, and every rule that allows the list is reinstalled -- which is the reason to keep them in one place.", "routes"},
	"DELETE /api/v1/source-lists/{id}":              {"Delete an address list", "Refused while a forwarding rule allows it: removing it would silently widen that rule to every source that can reach it.", "routes"},
	"POST /api/v1/routes/{id}/diagnostics/test-all": {"Probe every destination of a rule", "One answer per destination, including the ones that are out of the rotation, because a relay across several backends is not reachable or unreachable -- it is reachable for some share of its connections.", "diagnostics"},
